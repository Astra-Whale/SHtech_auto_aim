...
bool PredictorAdaptiveEKF::predict(Detection_pack &data, RobotCmd &send, cv::Mat &im2show, bool is_show)
{
    ...
    for (auto &d : detections)
    {
        ...
        { // 不能随意修改，否则会数组越界0-5
            ...
            if (last_shoot && d.tag_id == last_sbbox.tag_id)
                dead_buffer = 0; // 如果存在上一次击打装甲板同 id 且有颜色的，表示车辆存活，清空灰色缓冲
    ...
    if (!selected && last_shoot)
    {
        for (auto &d : new_detections)
        {
            ...
            if (last_shoot && (center.inside(get_ROI(last_sbbox)) || is_same_armor_by_distance(last_m_pw, d, R_IW)))
            {
                ...
                need_init = false;
                if (DEBUG)
                    std::cout << "selected same armor!" << std::endl;
                break;
            }
        }
    }
    ...
    if (DEBUG)
        std::cout << "last_is_one_armor = " << last_is_one_armor << ", last_is_two_armors = " << last_is_two_armors << std::endl;
    ...
    if (same_id && !antitop)
    {
        ...
        for (i = 0; i < antitop_candidates.size(); ++i)
        {
            ...
            if (center.inside(get_ROI(antitop_candidates[i].bbox)) || is_same_armor_by_distance(antitop_candidates[i].last_m_pw, armor, R_IW))
            {
                ...
                last_yaw = antitop_candidates[i].last_yaw;
                if (DEBUG)
                    std::cout << "choose candidates." << std::endl;
    ...
    if (DEBUG)
        std::cout << "update ekf vector" << std::endl;
    if (need_init)
    ...
    {
        if (DEBUG)
            std::cout << "---\ttag_id = " << d.tag_id << "\t---" << std::endl;
        if (d.tag_id != armor.tag_id || d == armor)
            continue; // 如果装甲板id与当前装甲板不同或就是目标装甲板
        bool exist = false;
        auto center = points_center(d.pts);
        for (auto &ac : antitop_candidates)
        {
    ...
    for (long unsigned int i = 0; i < antitop_candidates.size(); ++i)
    {
        ...
        if (size2 > switch_armor_size_proportion * size1)
        {
            ...
            if (DEBUG)
                std::cout << "Switch armor due to size scale." << std::endl;
            switch_armor = true;
            Record r(armor);
            r.ekf = ekf;
            r.updated = true;
    ...
    if (distant)
        send.shoot_mode += static_cast<uint8_t>(ShootMode::DISTANT);
    if (antitop)
        send.shoot_mode += static_cast<uint8_t>(ShootMode::ANTITOP);
    if (switch_armor)
        send.shoot_mode += static_cast<uint8_t>(ShootMode::SWITCH);
...
void PredictorAdaptiveEKF::match_armors(bbox_t &armor, bool &selected, const std::vector<bbox_t> &detections, const Eigen::Matrix3d &R_IW, const bool right)
{
    ...
    if (temp_armors.size() == 2)
    {
        ...
        if (flag)
            std::cout << "[Error] in match_armors." << std::endl;
        if (right)
        {
